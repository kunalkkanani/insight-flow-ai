...
const TABS: { id: Tab; label: string; icon: React.ReactNode }[] = [
  { id: "overview",  label: "Overview",   icon: <Database className="w-5 h-5" /> },
  { id: "charts",    label: "Charts",     icon: <BarChart3 className="w-5 h-5" /> },
  { id: "insights",  label: "Insights",   icon: <Lightbulb className="w-5 h-5" /> },
  { id: "chat",      label: "Ask AI",     icon: <MessageSquare className="w-5 h-5" /> },
  { id: "logs",      label: "Agent Logs", icon: <ScrollText className="w-5 h-5" /> },
];
    ...
    <div className="w-full max-w-7xl mx-auto space-y-6 animate-slide-up">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={onReset}
            className="text-base text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white border border-slate-200 dark:border-slate-700 hover:border-slate-400 dark:hover:border-slate-500 px-4 py-2 rounded-xl transition-all"
          >
            ← New Analysis
          </button>
        </div>
      ...
      <div className="flex gap-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-1 overflow-x-auto shadow-sm">
        {TABS.map((tab) => (
          <button
            ...
            className={clsx(
              "flex items-center gap-2 px-5 py-2.5 rounded-lg text-base font-medium whitespace-nowrap transition-all",
              activeTab === tab.id
      ...
      {report.errors.length > 0 && (
        <div className="bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/20 rounded-xl p-4">
          <p className="text-rose-600 dark:text-rose-400 font-medium text-base mb-2">
            {report.errors.length} non-fatal error(s) during analysis:
          </p>
          <ul className="space-y-1">
            {report.errors.map((e, i) => (
              <li key={i} className="text-rose-500 dark:text-rose-300 text-sm font-mono">{e}</li>
            ))}
          </ul>
        </div>
      )}
